...
async function _load(userId) {
  ...
  const myProducts = (allProducts || []).filter(p => p.createdBy === userId);
  const myProductMap = Object.fromEntries(myProducts.map(p => [p.id, p]));

  return (allVariants || [])
    .filter(v => myProductMap[v.productId])
    .map(v => ({
      ...v,
      productName:   myProductMap[v.productId].name   || '—',
      productStatus: myProductMap[v.productId].status || '—',
      productSku:    myProductMap[v.productId].sku    || '—',
    }));
}
...
export async function render(container) {
  ...
  [['', 'All stock levels'], ['out', 'Out of stock (0)'], ['low', 'Low stock (1–5)'], ['in', 'In stock (>5)']].forEach(([val, label]) => {
  ...
  function applyFilter() {
    ...
    _renderTable(tableWrap, filtered, countEl);
  }
...
function _renderTable(wrap, rows, countEl) {
  ...
  if (rows.length === 0) {
    card.innerHTML = `
      <div class="empty-state">
        ...
        <div class="empty-state__desc">Adjust the filters above, or add variants to your products.</div>
      </div>`;
  ...
  table.innerHTML = `
    <thead>
      <tr>
        <th>Product</th><th>Variant SKU</th><th>Size</th>
        <th>Color</th><th>Weight</th><th>Stock</th><th>Status</th>
      </tr>
    </thead>
    <tbody></tbody>`;
    ...
    tr.innerHTML = `
      <td>
        <div class="product-name">
          ${esc(v.productName)}<small>${esc(v.productSku)}</small>
        </div>
      </td>
